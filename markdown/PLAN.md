...
| IIoT container (NodeRED/MQTT/Influx) | [db_module/db_sources/iiot_container/](../db_module/db_sources/iiot_container/) | 🟥 empty Dockerfile + compose, empty nodered/ |
| Oracle SQL queries | [db_module/db_sources/oracle_sql_query/](../db_module/db_sources/oracle_sql_query/) | ✅ `01_dw_ddl.sql` + `02_sp_dim_date.sql` + `apply_ddl.py` + `verify_dw.py` — **live-applied to AI03** |
| Supabase SQL | [db_module/db_sources/supabases_sql_query/query/](../db_module/db_sources/supabases_sql_query/query/) + [mock/](../db_module/db_sources/supabases_sql_query/mock/) | ✅ `01_schema.sql` + `02_master_data.sql` + `03_mock_data.sql` + `generate_mock_data.py` — unverified against live Supabase |
...
- [x] Write [db_module/db_sources/supabases_sql_query/query/01_schema.sql](../db_module/db_sources/supabases_sql_query/query/01_schema.sql) — 17 tables from CLAUDE.md §7 (PostgreSQL: `SERIAL`, `DECIMAL`, `TIMESTAMP`, CHECK constraints, 7 indexes for ETL).
- [x] Write [db_module/db_sources/supabases_sql_query/query/02_master_data.sql](../db_module/db_sources/supabases_sql_query/query/02_master_data.sql) — 1 line, 3 machines, 10 stages, 3 products, 15 BOM rows, 4 suppliers, 5 raw materials, 5 opening inventory rows. Sequence values synced with `setval()` after inserts.
- [x] Author [db_module/db_sources/supabases_sql_query/mock/generate_mock_data.py](../db_module/db_sources/supabases_sql_query/mock/generate_mock_data.py) — deterministic (seed=42), 30 days starting 2026-03-19, bias toward instrumented stages 1/5/8.
- [x] Run generator → [03_mock_data.sql](../db_module/db_sources/supabases_sql_query/mock/03_mock_data.sql) (~900 KB, 15 K lines). **Row counts within 2-4 % of CLAUDE.md §5 targets:**
...
- [x] **Live-applied against Supabase** via [apply_supabase.py](../db_module/db_sources/supabases_sql_query/apply_supabase.py). All 3 files in one transaction (916 KB total). Row counts verified from live DB match generator output exactly.
...
The NodeRED + Mosquitto + InfluxDB 2.0 stack already runs on AWS. We **consume** it — we do not rebuild it locally. The local [db_module/db_sources/iiot_container/](../db_module/db_sources/iiot_container/) directory is now reference-only (keep empty compose as documentation of the remote layout, or delete).
...
- [ ] Decide whether `db_module/db_sources/iiot_container/` is kept as documentation or removed (recommend keeping a README + a copy of the NodeRED flow JSON for reproducibility).
...
- [x] Write [db_module/db_sources/oracle_sql_query/01_dw_ddl.sql](../db_module/db_sources/oracle_sql_query/01_dw_ddl.sql) — 5 STG + 5 DIM + 5 FACT tables, 9 sequences for surrogate keys, 5 reporting indexes, idempotent teardown block at top. Oracle 10g compatible (no `GENERATED AS IDENTITY`).
- [x] Write [db_module/db_sources/oracle_sql_query/02_sp_dim_date.sql](../db_module/db_sources/oracle_sql_query/02_sp_dim_date.sql) — `SP_LOAD_DIM_DATE(start, days)` using ISO-week anchoring for locale-safe weekend detection, English month names via `NLS_DATE_LANGUAGE=ENGLISH`, `MERGE` for idempotency.
- [x] Write [db_module/db_sources/oracle_sql_query/apply_ddl.py](../db_module/db_sources/oracle_sql_query/apply_ddl.py) — splits on `;` for SQL and `/` on own line for PL/SQL blocks, fails fast with rollback.
- [x] Write [db_module/db_sources/oracle_sql_query/verify_dw.py](../db_module/db_sources/oracle_sql_query/verify_dw.py) — asserts all 15 tables + 9 sequences exist.
...
- [x] **DIM_MACHINE / PRODUCT / STAGE / MATERIAL seeded from Supabase** via [seed_dims.py](../db_module/db_sources/oracle_sql_query/seed_dims.py) — first working cross-system ETL (Supabase → Oracle). Loaded 3 / 3 / 10 / 5 rows respectively. Source IDs preserved in `*_src_id` columns for fact-loader lookup.
...
- [x] [db_module/db_sources/oracle_sql_query/03_sp_fact_loaders.sql](../db_module/db_sources/oracle_sql_query/03_sp_fact_loaders.sql) — `FN_CALC_OEE` + `SP_LOAD_FACT_OEE` / `_QUALITY` / `_MAINTENANCE` / `_PRODUCTION`. All idempotent (DELETE-by-date then INSERT). Oracle 10g compatibility: SP_LOAD_FACT_PRODUCTION uses cursor-loop + sequence NEXTVAL (not INSERT…SELECT, which throws ORA-02287 on 10g).
- [x] Applied live against AI03 via [apply_ddl.py](../db_module/db_sources/oracle_sql_query/apply_ddl.py). All 5 objects (1 function + 4 procedures) compile `VALID`.
...
- [x] **[B]-4 Reporting queries** — [04_reporting_queries.sql](../db_module/db_sources/oracle_sql_query/04_reporting_queries.sql) with all 5 queries from CLAUDE.md §10. Q1/Q2/Q4 live-verified against populated DW (Q3 needs FACT_INVENTORY; Q5 needs multi-week data).
- [x] **[B]-5 Truncate + Reload script** — [05_truncate_and_reload.sql](../db_module/db_sources/oracle_sql_query/05_truncate_and_reload.sql). TRUNCATEs facts → dims (preserves DIM_DATE), drops + recreates sequences (Oracle 10g can't `ALTER SEQUENCE RESTART`), then driver comment points at `seed_dims.py` + SP re-run loop.
- [x] **FACT_INVENTORY pipeline** — [06_inventory_pipeline.sql](../db_module/db_sources/oracle_sql_query/06_inventory_pipeline.sql) adds `STG_INVENTORY` (5-row snapshot) + `STG_MATERIAL_CONSUMPTION` (per-date aggregates) + `SP_LOAD_FACT_INVENTORY`. Two new extract tasks added to [etl_supabase_to_oracle](../db_module/pipeline/airflow/dags/etl_supabase_to_oracle.py). `sp_load_fact_inventory` wired into [sp_load_dw](../db_module/pipeline/airflow/dags/sp_load_dw.py) as a 5th parallel task. Live-verified for 2026-04-17 → 5 rows (one per material) with opening/consumed/closing math working (e.g. Lead closing 5000 kg, consumed 67.6 kg, opening 5067.6 kg).
...
- **2026-04-19 — Migration to NEW_ARCHITECTURE complete.** Scope down จาก OEE-focused over-engineering เป็น Process Performance POC (single line, 3 machines, 15-min cadence). Dropped: material/inventory/BOM/supplier/stage domain, maintenance_log, FACT_OEE/FACT_INVENTORY/FACT_MAINTENANCE, DIM_STAGE/DIM_MATERIAL, FN_CALC_OEE. Added: `DIM_METRIC` (sensor catalog, 6 rows seeded), `FACT_SENSOR` (15-min aggregate per machine×metric×window). Simplified OLTP จาก 17 เหลือ 6 tables (production_line/machine/product/production_order/production_batch/qc_record). Changed SP strategy from DELETE-by-date → merge-by-key เพื่อรองรับ 15-min increments (DELETE-by-date จะล้าง FACT ทั้งวันในแต่ละ 15-min run). Live-verified: 5 production + 5 quality + 520 sensor rows loaded end-to-end; FastAPI เพิ่ม 8 dashboard endpoints ใหม่ (ลบของเก่า 6 endpoint OEE/maintenance/inventory); Streamlit 4 tabs (Production / Quality / Sensor per batch / Machine status 15-min). Python scripts renamed: `apply_ddl.py` → `run_sql_file.py`, `seed_dims.py` → `sync_dimensions_from_supabase.py`, `verify_dw.py` → `verify_warehouse_schema.py`; SQL files moved into `query/` subdir matching Supabase pattern. Deleted: `06_inventory_pipeline.sql`, `FN_CALC_OEE`, `SP_LOAD_FACT_OEE/MAINTENANCE/INVENTORY`. Dead code cleanup: removed `InfluxConnector.query_records()` + `.write()` (no callers). See [claude_track/NEW_PLAN.md](NEW_PLAN.md) สำหรับ migration detail.
